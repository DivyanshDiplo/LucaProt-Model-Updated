...
from transformers import EsmForProteinFolding, AutoTokenizer  # Added this line
...
if __name__ == "__main__":
    ...
    # Add tokenizer for proper sequence handling
    tokenizer = AutoTokenizer.from_pretrained("facebook/esmfold_v1")

    model = model.eval()

    # Handle chunk size setting with compatibility check
    if hasattr(model, 'set_chunk_size'):
        model.set_chunk_size(args.chunk_size)
    elif hasattr(model, 'trunk') and hasattr(model.trunk, 'set_chunk_size'):
        model.trunk.set_chunk_size(args.chunk_size)

    # Device configuration with compatibility
    if args.cpu_only:
